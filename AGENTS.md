...
GET /download/<hash>[?start&end] ─ exportRequested ──►  onExportRequested
  (holds responder, defers)          (signal)             └─ IClipExporter (ffmpeg) → downloads/.exports/<id>.mp4
  └─ serves MP4 + deletes file ◄── exportFinished ─────  emits when ffmpeg finishes
                                  (id,path,ok,err)

PlaybackProcessor logs [login-ok] [download] [stream-ready] [stream-error] [login-fail] [export] [export-ready] [export-error] → stdout/stderr
```
...
| `src/domain/I{Authenticator,PlaybackDownloader,DashPackager,ClipExporter,StreamCacheRepository}.h` | Qt-free port interfaces. Async ones expose `std::function` callbacks. `IClipExporter` remuxes a DASH manifest into one (optionally trimmed) MP4.                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                 |
...
| `src/infra/Config.h`                                                                               | `kDefaultApiKey`, `kDefaultPort`, `kDefaultLoginIdleSeconds`, `kDefaultMaxDownloadsSizeBytes` (2 GB), `kDefaultHashAlgorithm` (`"blake2s-128"`).                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                 |
| `src/adapter/http/HttpListener.{h,cpp}`                                                            | The entire HTTP boundary, on its own `QThread`. Owns `QHttpServer`/`QTcpServer` (created in `started()`); serves `POST /playback` (constant-time API-key check + inline `QJson`↔domain codec → `playbackRequested` signal), `GET /playback?id=` (reads a local status + URL mirror), `GET /dash/<hash>/<file>` (static MPD/segments, emits a single `keyAccessed` touch per served file); `GET /download/<hash>` (deferred MP4 export — holds the `QHttpServerResponder`, emits `exportRequested`, completes from `onExportFinished`). Built from `HttpListenerConfig` (which carries the hash-algorithm name); **owns** its `IHasher` (built via `QtHasher::fromName`). Holds no cache — the ready manifest URL arrives via the `statusChanged` signal.                                                                         |
| `src/adapter/processor/PlaybackProcessor.{h,cpp}`                                                  | Pipeline FSM on its own `QThread`: login → download → ffmpeg-DASH → cache eviction. Built from `PlaybackProcessorConfig`; **owns** all its collaborators — builds the `IAuthenticator` (HCNetSDK), `LoginUseCase`, downloader/packager factories, and the `FileSystemStreamCache` internally (concrete types named only in the `.cpp`). Talks to `HttpListener` only via Qt signals/slots (emits `statusChanged(keyHex, status, url)` and `exportFinished(requestId, path, ok, err)`); also handles `onExportRequested` by running an `IClipExporter` (ffmpeg) on its thread. Marshals downloader/packager/exporter callbacks onto its thread with `QMetaObject::invokeMethod`. Prints `[login-ok]`/`[download]`/`[stream-ready]`/`[stream-error]`/`[login-fail]`/`[export]`/`[export-ready]`/`[export-error]` to stdout/stderr. |
...
POST /playback
  ...
  → 200 {"poll_url": "http://<host>:18068/playback?id=<hash>"}
...
```
GET /playback?id=<hash>
  → 200 {"status":"ready","url":"http://<host>:18068/dash/<hash>/manifest.mpd"}
  → 202 {"status":"pending"}
...
GET /download/<hash>?start=<sec>&end=<sec> → clip, offset-seconds from recording start
GET /download/<hash>?start=<sec>           → from <sec> to the end
  → 200 Content-Type: video/mp4
         Content-Disposition: attachment; filename="playback-<hash>[-<start>s-<end>s].mp4"
  → 400 {"status":"error",...}   bad <hash> (not 16 hex) or bad range (end<=start, non-numeric)
  → 404 {"status":"error",...}   playback not present / not yet Ready
  → 500 {"status":"error",...}   ffmpeg failed
```

A single browser-friendly GET. The DASH package (`downloads/<hash>/manifest.mpd`)
is the source — the playback must already be `Ready` (request it first via
`POST /playback`). The HTTP handler validates, then **defers the response**: it
holds the `QHttpServerResponder` and emits `exportRequested` to the processor,
which remuxes the manifest into a single MP4 via ffmpeg (`-c copy`, no re-encode;
`-ss`/`-t` for the trim) on its own thread, then signals back. Cuts are
**keyframe-bounded** (segment/GOP granularity), so a clip may be off by up to one
keyframe interval. The exported file is a throwaway under
`downloads/.exports/`, **served then deleted** (and swept on startup); it never
counts toward the downloads capacity cap, and its source package is protected
from eviction while the export runs.
...
| `--max-downloads-gb=<n>`     | `2`                      | Capacity cap for the downloads directory. Oldest DASH packages are evicted after each successful packaging until usage falls below the limit. Active jobs and hashes served within the last 60 s are never removed. |
...
POLL_URL=$(curl -s -X POST http://localhost:18068/playback \
...
curl -i -X POST http://localhost:18068/playback -d '{}'    # → 401
curl -i -X POST http://localhost:18068/playback -d '{}'    # → 401
